...
class ApplicationExitHandler : CommandHandler {
    override val method = DabMethod.APPLICATIONS_EXIT
    override suspend fun handle(request: DabRequest, executor: DeviceExecutor): DabResponse = runCatching {
        executorResponse(request, executor, executor.exitApplication(requireParam(request, "packageName")))
